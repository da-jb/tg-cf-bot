# tg-cf-bot
爬别人代码，魔改的适合自己的可以部署在cloudflare上的tg双向机器人-beat版
...
- **资料补全**：资料卡自动显示 `@用户名`，并从用户文本/说明中识别手机号，标注中国大陆、中国香港、中国台湾、美国/加拿大等常见归属地
...
### 8. 💾 D1 配置导入/导出
- **一键导出**：主管理员在“备份/通知”面板点击“导出配置”，Bot 会发送 JSON 备份文件
- **一键导入**：点击“导入配置”后发送 JSON 文本或导出的 `.json` 文件，即可恢复欢迎语、屏蔽词、自动回复、协管列表、过滤开关等配置
- **安全范围**：导入/导出只处理 D1 `config` 表中的业务配置，不包含 `BOT_TOKEN`、`ADMIN_GROUP_ID` 等 Worker 环境变量

### 9. 🌐 AI 翻译
- **可选开启**：主管理员在控制面板点击“AI 翻译”开启/关闭
- **话题内追加**：用户文本或媒体 caption 转发到管理员话题后，Bot 会追加一条中文翻译，不影响原消息
- **模型配置**：默认使用 Cloudflare Workers AI 的 `@cf/meta/llama-3.1-8b-instruct` 自动识别语言并翻译，目标语言默认 `中文`
